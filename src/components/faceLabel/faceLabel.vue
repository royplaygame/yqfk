...
<template>
  <div class="role">
    <div class="roleList">
      <div class="imgBox img" id="dv" @click="arrowClick">
      ...
      <ul class="showLabel">
       ...
       <el-radio v-model="radio" label="1">彩色图片</el-radio>
  <el-radio v-model="radio" label="2">红外图片</el-radio>
      <el-button @click="clearLabel" type="primary">清空标注</el-button>
      <el-button @click="getImg" type="primary">下一张</el-button>
    </div>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    getBaseImg(e) {
      ...
      self
        .getFaceImg({
          type: 1,
          img_type: parseInt(self.radio)
        })
        .then(data => {
          if (data.code == "0") {
            self.imgsrc =  data.result.img_url;
            self.imgName = data.result.img_name;
            self.points = data.result.points || [];
            for (let v of self.points) {
    ...
    ...mapActions(["getFaceImg", "getLandMark"])
  },
  // 监视
  watch: {}
};
